...
export class ResolutionRepository {
  ...
  async getTransportNetworkInfo(turnId: number): Promise<TransportNetworkUnit[]> {
    const unitAdjacencyInfoResult: TransportNetworkUnit[] = await this.pool.query(getTransportNetworkValidation, [turnId])
      .then((results: QueryResult<any>) => {
        return results.rows.map((result: TransportNetworkUnitResult) => {
          return <TransportNetworkUnit> {
            unitId: result.unit_id,
            transportables: result.adjacent_transportables && result.adjacent_transportables.map((unit: AdjacentTransportResult) => {
              return <AdjacentTransportable> {
                unitId: unit.unit_id,
                unitName: unit.unit_name
              }
            }),
            transports: result.adjacent_transports && result.adjacent_transports.map((unit: AdjacentTransportableResult) => {
              return <AdjacentTransport> {
                unitId: unit.unit_id,
                unitName: unit.unit_name
              }
            }),
            destinations: result.transport_destinations && result.transport_destinations.map((destination: TransportDestinationResult) => {
