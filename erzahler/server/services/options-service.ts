    ...
    if (pendingTurn) {
      ...
      if ([TurnType.SPRING_ORDERS, TurnType.ORDERS_AND_VOTES].includes(pendingTurn.turnType)) {
        if (playerCountry.builds > 0) {
          const buildTransferOptions: TransferCountry[] = await db.optionsRepo.getBuildTransferOptions(gameId, gameState.turnId);
        ...
        if (playerCountry.nukeRange !== null) {
          const techTransferOptions: TransferCountry[] = await db.optionsRepo.getTechOfferOptions(gameId, gameState.turnId);
          ...
          const techTransferOptions: TransferCountry[] = await db.optionsRepo.getTechReceiveOptions(gameId, gameState.turnId);
