...
import { ProvinceStatus, ProvinceType, ResolutionEvent, VoteType } from "../../models/enumeration/province-enums";
...
export class ResolutionService {
  ...
  async resolveTurn(turn: UpcomingTurn): Promise<void> {
    ...
    const countryHistories: CountryState[] = await db.gameRepo.getCountryState(turn.gameId, 0);
    ...
    if (turnsWithUnitOrders.includes(turn.turnType)) {
      ...
      unitMovementResults.forEach((result: UnitOrderResolution) => {
        ...
        let provinceHistory = provinceHistories.find((province: ProvinceHistoryInsert) => province.provinceId === finalPosition.provinceId);
        if (provinceHistory) {
        ...
        if ([OrderDisplay.DISBAND, OrderDisplay.NUKE].includes(result.orderType) || result.unit.status === UnitStatus.NUKED) {
          const countryHistory = countryHistories.find((country: CountryState) => country.countryId === result.unit.countryId);
          if (countryHistory) {
        ...
        if (result.orderType === OrderDisplay.NUKE) {
          const targetCountry = countryHistories.find((country: CountryState) => country.countryId === result.destination.controllerId);
        ...
        if ([OrderDisplay.MOVE, OrderDisplay.MOVE_CONVOYED].includes(result.orderType)
         ...
         && result.orderSuccess === false) {
          const bounceFound = provinceHistories.find((province: ProvinceHistoryInsert) => province.provinceId === result.destination.provinceId);
          if (!bounceFound) {
  ...
  async resolveUnitOrders(gameState: GameState, turn: UpcomingTurn): Promise<UnitOrderResolution[]> {
    ...
    orderGroups.support.filter((order: UnitOrderResolution) => order.unit.status === UnitStatus.ACTIVE)
    ...
    if (orderGroups.transport.length > 0 || orderGroups.moveTransported.length > 0) {
      const transportNetwork: TransportNetworkUnit[] = await db.resolutionRepo.getTransportNetworkInfo(gameState.turnId);

      // Stores filter logic for DRY operation
      const activeMoveTransported = orderGroups.moveTransported.filter((order: UnitOrderResolution) =>
        order.unit.status === UnitStatus.ACTIVE
      );
      const activeTransports = orderGroups.transport.filter((order: UnitOrderResolution) =>
      order.unit.status === UnitStatus.ACTIVE
      );
    ...
    // Movement
    const unresolvedMovement = orderGroups.move.filter((order: UnitOrderResolution) =>
      order.unit.status === UnitStatus.ACTIVE
    );
    const moveTransported = orderGroups.moveTransported.filter((order: UnitOrderResolution) =>
      order.unit.status === UnitStatus.ACTIVE
    );
  ...
  sortAndValidateUnitOrder(order: UnitOrderResolution, unitOptions: UnitOptionsFinalized[], orderGroups: UnitOrderGroups): void {
    const options: UnitOptionsFinalized | undefined = unitOptions.find((option: UnitOptionsFinalized) => option.unitId === order.unit.id);
    ...
    if (options === undefined) {
      ...
      if (!supportableUnitIds.includes(order.secondaryUnit.id)) {
        ...
        const supportDestinationIds =
          options.supportStandardDestinations[order.secondaryUnit.id].map((destination: OptionDestination) => destination.nodeId);
      ...
      if (!supportableUnitIds.includes(order.secondaryUnit.id)) {
        ...
        const supportDestinationIds =
          options.supportTransportedDestinations[order.secondaryUnit.id].map((destination: OptionDestination) => destination.nodeId);
      ...
      if (!transportableUnitIds.includes(order.secondaryUnit.id)) {
        ...
        const transportDestinationIds =
          options.transportDestinations[order.secondaryUnit.id].map((destination: OptionDestination) => destination.nodeId);
  ...
  resolveNuclearLaunch(order: UnitOrderResolution, orders: UnitOrderResolution[]) {
    const victim: UnitOrderResolution | undefined = orders.find((victim: UnitOrderResolution) => victim.origin.provinceId === order.destination.provinceId);
  ...
  resolveSupport(order: UnitOrderResolution, orders: UnitOrderResolution[], dependencies: OrderDependencies) {
    const supportedUnit: UnitOrderResolution | undefined = orders.find((unit: UnitOrderResolution) => unit.unit.id === order.secondaryUnit.id);
    if (supportedUnit) {
      ...
      const destinationNuked = orders.find((strike: UnitOrderResolution) =>
      ...
      const validCuts = orders.filter((attack: UnitOrderResolution) =>
      ...
      if (validCuts.length > 0) {
        order.primaryResolution = order.primaryResolution ? `${order.primaryResolution}. Failed: Support Cut` : 'Failed: Support Cut';
        order.supportCut = true;
      }

      if (!order.supportCut) {
        const defender: UnitOrderResolution | undefined = orders.find((defender: UnitOrderResolution) => defender.origin.provinceId === order.destination.provinceId);
        if (defender) {
          if ([OrderDisplay.HOLD, OrderDisplay.AIRLIFT, OrderDisplay.CONVOY, OrderDisplay.SUPPORT, OrderDisplay.SUPPORT_CONVOYED].includes(defender.orderType)
            && defender.unit.countryId === order.unit.countryId) {
            order.valid = false;
            order.primaryResolution = `Invalid Order: Can't Self-Dislodge`;
          }

          if ([OrderDisplay.MOVE, OrderDisplay.MOVE_CONVOYED].includes(defender.orderType)
            && defender.unit.countryId === order.unit.countryId
            && order.valid) {
  ...
  createTransportPaths(
    ...
    const compliantTransportsIds = transportOrders.filter((transport: UnitOrderResolution) =>
    ...
    let initialTransportIds = transportNetwork.find((transported: TransportNetworkUnit) =>
      transported.unitId === order.unit.id
  ...
  extendTransportPath(
    ...
    const currentTransport = compliantTransports.find((transport: TransportNetworkUnit) => transport.unitId === currentLinkId);
    if (currentTransport) {
      const newCommitted = committed.slice();
      newCommitted.push(currentLinkId);

      const destination = currentTransport.destinations.find((destination: TransportDestination) =>
        destination.nodeId === order.destination.nodeId
      );
      if (destination) {
  ...
  getNextTransports(
    ...
    const nextTransports = compliantTransports.find((transport: TransportNetworkUnit) =>
      transport.unitId === currentUnitId
    )
    ?.transports
    .filter((transport: AdjacentTransport) => !committedTransportIds.includes(transport.unitId))
  ...
  checkTransportSuccess(transportOrder: UnitOrderResolution, moveStandardOrders: UnitOrderResolution[]) {
    const challengers = moveStandardOrders.filter((challengingOrder: UnitOrderResolution) =>
    ...
    const transportPowerSummary = this.createTransportPowerConflictSummary(
      transportOrder.power,
      challenges,
      maxPower
    );
    ...
    challengers.forEach((challenger: UnitOrderResolution) => {
      this.resolveTransportChallenge(
        transportOrder.power,
        challenger,
        challenges,
        maxPower
  ...
  resolveMovement(
    ...
    const nuclearStrike = unitOrders.find((strike: UnitOrderResolution) =>
      strike.orderType === OrderDisplay.NUKE && strike.destination.provinceId === order.destination.provinceId
    );
    if (nuclearStrike) {
      order.valid = false;
    ...
    const holdingChallenger = unitOrders.find((challenger: UnitOrderResolution) =>
    ...
    if (order.orderSuccess) {
      const leavingUnit = unitOrders.find((leavingUnit: UnitOrderResolution) =>
        leavingUnit.origin.provinceId === order.destination.provinceId
      );
  ...
  checkDependency(orderId: number, dependencies: OrderDependencies, unitOrders: UnitOrderResolution[]) {
    ...
    if (independency) {
      ...
      const dependency = unitOrders.find((order: UnitOrderResolution) => order.orderId === dependencies.dependency[orderId].orderId);
      if (dependency) {
  ...
  getFinalPosition(result: UnitOrderResolution): OrderResolutionLocation {
    ...
    if ((result.unit.status === UnitStatus.ACTIVE  || result.unit.status === UnitStatus.RETREAT || result.unit.status === UnitStatus.DETONATED)
      && (result.orderType === OrderDisplay.MOVE || result.orderType === OrderDisplay.MOVE_CONVOYED || result.orderType === OrderDisplay.NUKE)
      && result.orderSuccess) {
  ...
  createProvinceHistory(result: UnitOrderResolution, finalPosition: OrderResolutionLocation, turn: UpcomingTurn): ProvinceHistoryInsert {
    return {
  ...
  resolveControllerId(result: UnitOrderResolution, finalPosition: OrderResolutionLocation, turn: UpcomingTurn): number {
    if (
      ...
      && turn.hasCaptures && result.unit.canCapture)
      ||
      ([ProvinceType.COAST, ProvinceType.INLAND, ProvinceType.ISLAND].includes(finalPosition.provinceType)
  ...
  resolveProvinceStatus(result: UnitOrderResolution, finalPosition: OrderResolutionLocation, turn: UpcomingTurn): ProvinceStatus {
    ...
    if ([ProvinceType.COAST, ProvinceType.INLAND, ProvinceType.ISLAND].includes(finalPosition.provinceType)
      && [ProvinceStatus.ACTIVE, ProvinceStatus.DORMANT, ProvinceStatus.BOMBARDED]
      && turn.hasCaptures && result.unit.canCapture) {
    ...
    if ([ProvinceType.COAST, ProvinceType.INLAND, ProvinceType.ISLAND].includes(finalPosition.provinceType)
      && [ProvinceStatus.ACTIVE, ProvinceStatus.DORMANT, ProvinceStatus.BOMBARDED]
      && turn.hasCaptures && result.unit.type === UnitType.WING) {
  ...
  getOrCreateProvinceHistory(provinceHistories: ProvinceHistoryInsert[], finalPosition: OrderResolutionLocation): ProvinceHistoryInsert {
    let provinceHistory = provinceHistories.find((province: ProvinceHistoryInsert) => province.provinceId === finalPosition.provinceId);
